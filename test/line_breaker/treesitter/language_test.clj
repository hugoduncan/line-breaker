  ...
  (testing "find-library-path"
    (testing "uses env var path when set and file exists"
      (let [temp-file (fs/create-temp-file {:suffix ".dylib"})]
        (try
          (with-redefs
           [lang/get-native-loader-path
            (constantly nil)
            lang/get-env-lib-path
            (constantly (str temp-file))]
            (let [[path source] (#'lang/find-library-path)]
              (is (= :env-var source) "reports :env-var as the source")
              (is (= (str temp-file) (str path)) "returns the env var path")))
          (finally (fs/delete temp-file)))))
    (testing "falls back to classpath when env var path does not exist"
      (with-redefs
       [lang/get-native-loader-path
        (constantly nil)
        lang/get-env-lib-path
        (constantly "/nonexistent/lib.dylib")]
      ...
      (with-redefs
       [lang/get-native-loader-path
        ...
        (let [ex (try (#'lang/find-library-path) nil (catch ExceptionInfo e e))]
          ...
          (when ex
            (let [data (ex-data ex)]
              (is (contains? data :library) "ex-data includes :library")
              (is (contains? data :os) "ex-data includes :os")
              (is (contains? data :arch) "ex-data includes :arch")
              (is
               (contains? data :resource-path)
