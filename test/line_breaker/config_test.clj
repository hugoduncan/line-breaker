...
(deftest default-config-test
  (testing "default-config"
    ...
    (testing "has :extensions for Clojure files"
      (is
       (= [".clj" ".cljs" ".cljc" ".edn"] (:extensions config/default-config))))
...
(deftest find-config-file-test
  (testing "find-config-file"
      ...
      (testing "returns the config path"
        (with-temp-dir [root]
          (let [config-path (fs/path root ".line-breaker.edn")]
            ...
            (is (= (str config-path) (config/find-config-file root)))))))
      ...
      (testing "returns the parent config path"
        (with-temp-dir [root]
          (let [config-path (fs/path root ".line-breaker.edn")
            ...
            (spit (str config-path) "{}")
            (is (= (str config-path) (config/find-config-file child-dir)))))))
    ...
    (testing "when no config exists"
      (testing "returns nil"
        (with-temp-dir [root] (is (nil? (config/find-config-file root))))))))
...
(deftest deep-merge-test
  (testing "deep-merge"
    ...
    (testing "deeply merges nested maps"
      (is (= {:a {:b 1
                  :c 2}} (config/deep-merge {:a {:b 1}} {:a {:c 2}}))))
